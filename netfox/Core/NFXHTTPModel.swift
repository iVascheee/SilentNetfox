...
@objc public class NFXHTTPModel: NSObject {
    ...
    @objc public func saveData(_ dataString: String, to fileURL: URL) {
        do {
            ...
            NFX.sharedInstance().nfxPrint("[NFX]: Failed to save data to [\(fileURL)] - \(error.localizedDescription)")
        }
    }
    ...
    @objc public func readRawData(from fileURL: URL) -> Data? {
        do {
            ...
            NFX.sharedInstance().nfxPrint("[NFX]: Failed to load data from [\(fileURL)] - \(error.localizedDescription)")
            return nil
        }
    }
