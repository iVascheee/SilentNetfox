...
struct NFXPath {
    ...
    static func createNFXDirIfNotExist() {
        do {
            ...
            NFX.sharedInstance().nfxPrint("[NFX]: failed to create working dir - \(error.localizedDescription)")
        }
    }
    ...
    static func deleteNFXDir() {
        ...
        do {
            ...
            NFX.sharedInstance().nfxPrint("[NFX]: failed to delete working dir - \(error.localizedDescription)")
        }
    }
...
extension String {
    ...
    func appendToFileURL(_ fileURL: URL) {
        ...
        if #available(iOS 13.4, macOS 10.15.4, *) {
            do {
                ...
                NFX.sharedInstance().nfxPrint("[NFX]: Failed to append [\(self.prefix(128))] to \(fileURL), trying to create new file - \(error.localizedDescription)")
                write(to: fileURL)
            }
        } else {
    ...
    private func write(to fileURL: URL) {
        do {
            ...
            NFX.sharedInstance().nfxPrint("[NFX]: Failed to save [\(self.prefix(128))] to \(fileURL) - \(error.localizedDescription)")
        }
    }
